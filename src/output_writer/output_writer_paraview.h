...
public:
  ...
  OutputWriterParaview(std::shared_ptr<Discretization> discretization);
    ...
    OutputWriterParaview(std::shared_ptr<Discretization> discretization, Settings settings);

//! write current velocities to file, filename is output_<count>.vti
    void writeFile(double currentTime) {
        writeFile(currentTime, "broken");

    }

    void writeFile(double currentTime, const std::string& type);
